...
OUT="$ROOT/matrices-frontier/artifacts-wave4/$HYPOTHESIS_ID"
WORK="$RUNNER_TEMP/ssi-ordering-wave4-$HYPOTHESIS_ID"
mkdir -p "$OUT"
...
section "apply one subtractive constant mutation"
...
pat = re.compile(rf'(?m)^(\s*const\s+{re.escape(name)}\s*:\s*[^=;\n]+\s*=\s*)([^;\n]+)(;)')
hits=[]
for path in sorted(root.rglob('*.rs')):
    text=path.read_text()
    for m in pat.finditer(text):
        hits.append((path,m.group(2).strip()))
if len(hits) != 1:
    raise SystemExit(f'expected exactly one definition of {name}, found {hits}')
path, old = hits[0]
text=path.read_text()
out,count=pat.subn(lambda m:m.group(1)+value+m.group(3),text,count=1)
if count != 1:
...
set -e
cat "$OUT/setup.log"
if [[ $rc -ne 0 ]]; then write_meta setup "failed:$rc"; exit "$rc"; fi
...
if [[ $rc -ne 0 ]]; then write_meta candidate-build "failed:$rc"; exit "$rc"; fi
...
if [[ $rc -ne 0 || ! -f score.json ]]; then write_meta benchmark "failed:$rc"; exit "$rc"; fi
